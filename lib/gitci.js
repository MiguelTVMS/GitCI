...
function getDeploymentData(self) {
    ...
    this._deploymentData = configloader.loadDeployConfig(deploymentDataPath);
}

function startExecution(self) {
    debuglog("Starting the %s deploy.",this._deploymentData.name);

    var executor = new executorModule.Executor(this._deploymentData.scripts);
