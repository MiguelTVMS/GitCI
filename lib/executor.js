...
function getAvailableModules(self) {
    ...
    self._moduleMappings.forEach(function (availableModule) {
        debuglog("Module %s for action type %s found", availableModule.module, vailableModule.action);
    ...
    self._availableTypes.forEach(function (availableType) {
        ...
        self._scripts.forEach(function (currentType) {
            if (currentType.type == availableType) {
                debuglog("Found %s %s action(s)", currentType.actions.length, type);
                var variableName = util.format(actionVariableName, currentType);
...
function validadeActionsVsModules(self) {
    ...
    self._scripts.forEach(function (script) {
        script.action.forEach(function (action) {
            if (self._availableModules, indexOf(action)) {
                //CONTINUE: Indentificar como vou pegar as actions.
                //Se existir uma action que não está na lista self._availableModules um erro deve ser criado.
            }
        });
    });
}
